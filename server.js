...
app.post('/api/save-image', async (req, res) => {
    const { userId, urlDaImagem, promptUtilizado, materia, estilo, topicoEspecifico, nivelEducacional } = req.body;

    if (!userId || !urlDaImagem || !promptUtilizado) {
        return res.status(400).json({
            success: false,
            message: 'Dados incompletos para salvar imagem.'
        });
    }

    try {
        const imagemSalva = await Image.saveImage(userId, {
            urlDaImagem,
            promptUtilizado,
            materia,
            estilo,
        ...
        console.log('✅ Imagem salva no banco:', imagemSalva._id);
